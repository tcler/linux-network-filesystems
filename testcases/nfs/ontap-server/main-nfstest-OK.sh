...
read NIC clnt2addr < <(vm exec $nfsclnt2 -- get-network-info.sh | grep ${lservaddr%.*})
...
mkdir -p $resdir
{
  ...
  vmrunx - $nfsclnt -- nfstest_delegation --server=${NETAPP_NAS_HOSTNAME} --export=$expdir --nfsversion=4.2 --client $clnt2addr --client-nfsvers=4.0,4.1,4.2 $TESTS;
  #stopvms
  exFail=0
  trun -x0 nfstest-result-check.sh $exFail $resdir/std.log
} &> >(tee $resdir/std.log)
